    ...
    <div class="mainParentConfig">
        ...
        <div class="configBox">
            ...
            <div class="tabContent" id="Roof" style="display: block;">
                <div class="rows"
                    style="width: 100%;display: grid; margin-top: 50px; align-items: center;justify-content: center;">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('keydown', function(event) {
                ...
                // Handle copy command (Ctrl + C or Command + C)
                if ((event.ctrlKey || event.metaKey) && event.key === 'c') {
                    console.log("Copy command detected");
                    const activeObject = activeCanvas.getActiveObject();
                    if (activeObject) {
                        copiedObject = fabric.util.object.clone(activeObject); // Clone the active object
                        console.log("Copied Object:", copiedObject); // Log the copied object for debugging
                    }
                }
                ...
                if ((event.ctrlKey || event.metaKey) && event.key === 'v') {
                    ...
                    if (copiedObject) {
                        console.log("Attempting to clone copiedObject:", copiedObject);

                        // Use enlivenObjects if the copiedObject contains images or other complex data
                        fabric.util.enlivenObjects([copiedObject], function(enlivenedObjects) {
                            if (enlivenedObjects.length > 0) {
                                const newObject = enlivenedObjects[0]; // Get the enlivened object
                                newObject.set({
                                    left: newObject.left + 10, // Offset for pasted object
                                    top: newObject.top + 10
                                });
                                activeCanvas.add(newObject); // Add new object to activeCanvas
                                activeCanvas.renderAll(); // Render the updated canvas
                            } else {
                                console.error(
                                    "New object could not be created from the copied object.");
                            }
                        });
                    }
                }
